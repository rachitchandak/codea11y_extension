...
export const INTENT_SYSTEM_PROMPT = `You are an expert accessibility audit planner. Given a user query and a project file tree, first decide whether the user is actually requesting an accessibility audit.
...
export const AUDIT_SYSTEM_PROMPT = `You are an expert WCAG 2.1 accessibility auditor. You will analyze source code for accessibility issues.
...
export const VALIDATE_SYSTEM_PROMPT = `You are a senior accessibility expert acting as a validation judge. Your role is to review accessibility audit findings and assess their accuracy.
...
export const REPORT_JUDGE_SYSTEM_PROMPT = `You are a senior accessibility expert acting as the final judge for a consolidated WCAG report.
...
export function buildFilePrimePrompt(
  basename: string,
  content: string,
  auditableInventory: string
): string {
...
export function buildGuidelineCheckPrompt(wcagId: string, description: string): string {
...
export function buildReportJudgePrompt(args: {
  targetFilePath: string;
  dependencyPaths: string[];
  files: Array<{ filePath: string; role: "target" | "dependency"; content: string }>;
  candidates: Array<{
    candidateId: string;
    filePath: string;
    guideline: string;
