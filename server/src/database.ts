...
function createAuditTables(db: Database): void {
    db.run(`
...
function createAuthTables(db: Database): void {
    db.run(`
...
function ensureColumn(db: Database, tableName: string, columnName: string, alterSql: string): void {
...
function ensureDefaultAdmin(db: Database): void {
...
function createDefaultAdmin(db: Database): void {
    const bcrypt = require('bcryptjs');
    const adminEmail = process.env.DEFAULT_ADMIN_EMAIL || 'admin@codea11y.com';
    const adminPassword = process.env.DEFAULT_ADMIN_PASSWORD || (!isProduction ? 'admin123' : '');
    ...
    db.run(
        `INSERT INTO users (email, password_hash, is_approved, is_admin, force_password_change) VALUES (?, ?, 1, 1, 1)`,
