...
const threadCache = new Map(); // sessionId -> threadId
// Helper to get Azure client for a user
function getAzureClient(userId) {
    ...
    const [apiKey, resourceName, deploymentName, workerDeploymentName, apiVersion] = row;
...
function logApiCall(sessionId, userId, agentType, endpoint, requestData, responseData, durationMs, tokensUsed) {
    const db = (0, database_1.getDatabase)();
...
router.post('/chat', auth_1.authenticate, async (req, res) => {
    ...
    try {
        ...
        logApiCall(sessionId, userId, agentType, '/chat', { messages: messages.map((m) => ({ role: m.role, content: m.content })) }, { content, usage }, Date.now() - startTime, usage?.total_tokens);
        res.json({
            content,
            usage,
            sessionId
...
// ==========================================
// Create Assistant
router.post('/assistants/create', auth_1.authenticate, async (req, res) => {
    const startTime = Date.now();
    const userId = req.user.id;
    ...
    try {
        ...
        logApiCall(sessionId, userId, 'manager', '/assistants/create', { name, instructions }, { assistantId: assistant.id }, Date.now() - startTime);
...
// Create Thread
router.post('/assistants/:assistantId/threads', auth_1.authenticate, async (req, res) => {
    const startTime = Date.now();
    const userId = req.user.id;
    ...
    try {
        ...
        logApiCall(sessionId, userId, 'manager', '/threads/create', { assistantId }, { threadId: thread.id }, Date.now() - startTime);
...
router.post('/threads/:threadId/messages', auth_1.authenticate, async (req, res) => {
    const startTime = Date.now();
    const userId = req.user.id;
    ...
    try {
        ...
        logApiCall(sessionId, userId, 'manager', '/threads/messages/create', { threadId, content }, { messageId: message.id }, Date.now() - startTime);
...
// Create Run
router.post('/threads/:threadId/runs', auth_1.authenticate, async (req, res) => {
    const startTime = Date.now();
    const userId = req.user.id;
    const { threadId } = req.params;
    ...
    try {
        ...
        logApiCall(sessionId, userId, 'manager', '/threads/runs/create', { threadId, assistantId }, { runId: run.id, status: run.status }, Date.now() - startTime);
...
// Get Run Status
router.get('/threads/:threadId/runs/:runId', auth_1.authenticate, async (req, res) => {
    const userId = req.user.id;
    ...
    try {
        const run = await azureConfig.client.beta.threads.runs.retrieve(runId, { thread_id: threadId });
        res.json({
            runId: run.id,
            status: run.status,
...
router.get('/threads/:threadId/messages', auth_1.authenticate, async (req, res) => {
    const userId = req.user.id;
    const { threadId } = req.params;
    const azureConfig = getAzureClient(userId);
    if (!azureConfig) {
    ...
    try {
        ...
        const formattedMessages = messages.data.map(m => ({
            id: m.id,
            role: m.role,
            content: m.content.map(c => c.type === 'text' ? c.text?.value : null).filter(Boolean)
        }));
...
// Stream Run (SSE)
router.post('/threads/:threadId/runs/stream', auth_1.authenticate, async (req, res) => {
    const startTime = Date.now();
    const userId = req.user.id;
    const { threadId } = req.params;
    const { sessionId, assistantId } = req.body;
    ...
    try {
        ...
        logApiCall(sessionId, userId, 'manager', '/threads/runs/stream', { threadId, assistantId }, { response: fullResponse }, Date.now() - startTime);
        res.end();
    }
    catch (error) {
...
// Delete Assistant
router.delete('/assistants/:assistantId', auth_1.authenticate, async (req, res) => {
    const userId = req.user.id;
    const { assistantId } = req.params;
    const { sessionId } = req.body;
    const azureConfig = getAzureClient(userId);
...
// ==========================================
router.post('/assistants/chat', auth_1.authenticate, async (req, res) => {
    const startTime = Date.now();
    const userId = req.user.id;
    ...
    try {
        ...
        // Poll for completion
        let runStatus = await azureConfig.client.beta.threads.runs.retrieve(run.id, { thread_id: threadId });
        while (runStatus.status !== 'completed' && runStatus.status !== 'failed') {
            await new Promise(resolve => setTimeout(resolve, 1000));
            runStatus = await azureConfig.client.beta.threads.runs.retrieve(run.id, { thread_id: threadId });
        }
        ...
        logApiCall(sessionId, userId, 'manager', '/assistants/chat', { threadId, message }, { response: responseText }, Date.now() - startTime);
