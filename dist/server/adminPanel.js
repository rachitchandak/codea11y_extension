...
exports.mountAdminPanel = mountAdminPanel;
 ...
 *  Mount admin routes                                                 *
 * ------------------------------------------------------------------ */
function mountAdminPanel(app, onProviderChange) {
    app.use("/admin", basicAuth);
    /* ── Dashboard: list all sessions ──────────────────────────────── */
    app.get("/admin", (_req, res) => {
        const sessions = (0, db_1.getAllSessions)();
    ...
    app.get("/admin/session/:id", (req, res) => {
        ...
        const session = (0, db_1.getSessionById)(sessionId);
        if (!session) {
            res.status(404).send("Session not found");
            return;
        }
        const calls = (0, db_1.getLlmApiCallsBySession)(sessionId);
     ...
     * ================================================================ */
    const providerTypeOptions = Object.keys(providers_1.PROVIDER_CATALOG)
        .map((pt) => providers_1.PROVIDER_CATALOG[pt])
        .map((c, i) => ({ type: Object.keys(providers_1.PROVIDER_CATALOG)[i], label: c.label }));
    /* ── List providers ────────────────────────────────────────────── */
    app.get("/admin/providers", (req, res) => {
        const providers = (0, db_1.getAllLlmProviders)();
        ...
        if (providers.length === 0) {
            ...
            for (const p of providers) {
                const catalogEntry = providers_1.PROVIDER_CATALOG[p.providerType];
                const typeLabel = catalogEntry?.label || p.providerType;
                let config = {};
                try {
                    config = JSON.parse(p.configJson);
                }
                catch { /* ignore */ }
    ...
    /* ── New provider form ─────────────────────────────────────────── */
    app.get("/admin/providers/new", (_req, res) => {
    ...
    app.post("/admin/providers", (req, res) => {
        try {
            ...
            const catalog = providers_1.PROVIDER_CATALOG[providerType];
            if (!catalog) {
    ...
    /* ── Edit provider form ────────────────────────────────────────── */
    app.get("/admin/providers/:id/edit", (req, res) => {
        const id = Number(req.params.id);
        const provider = (0, db_1.getLlmProviderById)(id);
        if (!provider) {
    ...
    /* ── Update provider ───────────────────────────────────────────── */
    app.post("/admin/providers/:id/update", (req, res) => {
        try {
            const id = Number(req.params.id);
    ...
    /* ── Activate provider ─────────────────────────────────────────── */
    app.post("/admin/providers/:id/activate", (req, res) => {
        try {
            const id = Number(req.params.id);
            (0, db_1.setActiveLlmProvider)(id);
            onProviderChange();
    ...
    /* ── Delete provider ───────────────────────────────────────────── */
    app.post("/admin/providers/:id/delete", (req, res) => {
        try {
            const id = Number(req.params.id);
            const provider = (0, db_1.getLlmProviderById)(id);
            if (provider?.isActive) {
    ...
    function providerFormHtml(currentName = "", currentType = "", currentConfig = {}, editId) {
        ...
        const typeOptions = providerTypeOptions
            .map((o) => `<option value="${escapeHtml(o.type)}" ${o.type === currentType ? "selected" : ""}>${escapeHtml(o.label)}</option>`)
            .join("");
        // Build per-provider field sets (hidden/shown by JS)
        let fieldSets = "";
        for (const [pt, catalog] of Object.entries(providers_1.PROVIDER_CATALOG)) {
